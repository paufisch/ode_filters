...
class TestMaternCompanionForm:
    ...
    def test_returns_correct_shapes(self):
        ...
        l, q = 1.0, 2
        F, L, q_coeff = _matern_companion_form(l, q)
        D = q + 1
    ...
    def test_F_has_super_diagonal_ones(self):
        ...
        l, q = 2.0, 2
        F, _, _ = _matern_companion_form(l, q)
        D = q + 1

        for i in range(D - 1):
            assert F[i, i + 1] == pytest.approx(1.0), (
                f"F[{i}, {i+1}] should be 1.0"
            )

    def test_L_has_one_in_last_position(self):
        """Test that L vector has 1 in last position, zeros elsewhere."""
        l, q = 1.5, 3
        _, L, _ = _matern_companion_form(l, q)
        D = q + 1
    ...
    def test_rejects_non_positive_length_scale(self):
        ...
        with pytest.raises(ValueError, match="Length scale l must be positive"):
...
class TestMaternPrior:
    ...
    def test_constructor_basic(self):
        ...
        prior = MaternPrior(q=2, d=1, l=1.0)
        assert prior.q == 2
        assert prior._dim == 1
